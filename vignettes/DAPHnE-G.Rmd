...
# Data
DAPHnE-G data repository consists of 8 tables (5 included in the package plus 2 external): a description of the ecotypes; a collection of environmental parameters and corresponding metadata; the phenotypic traits also with metadata. 
...
It is also possible to choose to use a leave-one-chromosome-out (loco) approach. In this case in the data preprocessing it will be created two genotype files for each chromosomes. Those files will contain the subsets of all SNPs belonging to one specific chromosome and its complementary (all SNPs that don't belong to one chromosome).
...
In case of *loco* approach it will be performed one study for each chromosome, computing a kinship matrix from all the SNPs which don't belong to the trget chromosome.
